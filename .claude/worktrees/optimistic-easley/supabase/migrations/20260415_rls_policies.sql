...
-- bot_accounts
DROP POLICY IF EXISTS "bot_accounts_select_public"        ON bot_accounts;
DROP POLICY IF EXISTS "bot_accounts_insert_admin"         ON bot_accounts;
DROP POLICY IF EXISTS "bot_accounts_update_admin"         ON bot_accounts;
DROP POLICY IF EXISTS "bot_accounts_delete_admin"         ON bot_accounts;

-- bot_posts
DROP POLICY IF EXISTS "bot_posts_select_admin"            ON bot_posts;
DROP POLICY IF EXISTS "bot_posts_insert_admin"            ON bot_posts;
DROP POLICY IF EXISTS "bot_posts_update_admin"            ON bot_posts;
DROP POLICY IF EXISTS "bot_posts_delete_admin"            ON bot_posts;
...
-- =============================================================
-- SECTION 12: BOT_ACCOUNTS
-- Bot accounts are public sellers — their profiles are visible.
-- Only admin or service_role can create/modify bot accounts.
-- =============================================================

CREATE POLICY "bot_accounts_select_public" ON bot_accounts
  FOR SELECT
  USING (is_active = TRUE OR is_admin());

CREATE POLICY "bot_accounts_insert_admin" ON bot_accounts
  FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY "bot_accounts_update_admin" ON bot_accounts
  FOR UPDATE
  USING (is_admin());

CREATE POLICY "bot_accounts_delete_admin" ON bot_accounts
  FOR DELETE
  USING (is_admin());

-- =============================================================
-- SECTION 13: BOT_POSTS
-- Internal audit table — not surfaced in the public app.
-- Admin-only read; service_role creates posts during bot runs.
-- =============================================================

CREATE POLICY "bot_posts_select_admin" ON bot_posts
  FOR SELECT
  USING (is_admin());

CREATE POLICY "bot_posts_insert_admin" ON bot_posts
  FOR INSERT
  WITH CHECK (is_admin());

CREATE POLICY "bot_posts_update_admin" ON bot_posts
  FOR UPDATE
  USING (is_admin());

CREATE POLICY "bot_posts_delete_admin" ON bot_posts
  FOR DELETE
  USING (is_admin());
